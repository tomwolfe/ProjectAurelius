"""Aurelius Score v5.2 (S_A_v5.2) computation engine.

Formula:
    S_A = 0.3(sigma) + 0.2(E_des_barrier) + 0.2(SEI Homogeneity)
          + 0.2(MX_Synthesis_Score) - 0.1(GWP)

Where:
    sigma          = MLX-NA filter confidence score (Tier 1)
...
from aurelius.data.params import ForceFieldConfig
...
class AureliusScoringEngine:
    ...
    Accepts a ForceFieldConfig via constructor injection for clean
    dependency injection, avoiding singleton cache hacks and clear_cache().
    """
    ...
    def print_scorecard(self, score: AureliusScoreResult) -> str:
        ...
        lines = [
            ...
            f"{'-' * 60}",
            "  Component Scores:",
            f"    σ (MLX-NA filter):       {score.sigma_score:>6.1f}  x {score.weight_sigma}",
            f"    E_des_barrier:           {score.desolvation_score:>6.1f}  x {score.weight_desolvation}",
            f"    SEI Homogeneity:         {score.sei_homogeneity_score:>6.1f}  x {score.weight_sei}",
            f"    MX_Synthesis_Score:      {score.mx_synthesis_score:>6.1f}  x {score.weight_mx}",
            f"    GWP Penalty:            -{score.gwp_penalty:>5.1f}  x {score.weight_gwp}",
            f"{'-' * 60}",
            "  Tier Status:",
    ...
    def _compute_mx_synthesis_score(
        ...
        """
        mx_params = self._config.get_scoring()
