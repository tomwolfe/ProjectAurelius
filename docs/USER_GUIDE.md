...
[`architecture.md`](architecture.md). For domain tuning guidance, see
[`certification_protocol.md`](certification_protocol.md).
...
| `[ml]` | `numpy`, `scikit-learn`, `joblib`, `scipy`, `xgboost` | ML surrogate, kernel tuning |
| `[web]` | `fastapi`, `uvicorn`, `diskcache` | Optional API server |
...
`property`, `value`). Runs a local Nelder-Mead optimizer (no external services
...
Validate that a kernel JSON file contains all required fields and is
...
The engine's modular hybrid oracle generalizes to other domains by retuning
HOMO/LUMO targets and GC fragment contributions. Use the `tune` command
to recalibrate for your domain:

```bash
aurelius tune my_experiments.csv --output my_kernel.json
```

For organic electronics, adjust HOMO/LUMO targets for the desired charge-transport
characteristics. For drug discovery, replace the electrolyte viability filter
with Lipinski/Ro5 rules and recalibrate the GC fragment library.
