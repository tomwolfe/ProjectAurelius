...
def test_active_learning_queue_atomic_append():
...
def test_all_results_atomic_append():
...
def test_screened_fingerprints_atomic_append():
...
def test_concurrent_add_and_clear():
    ...
    # After clear, state should be empty
    assert len(state._all_results) == 0
    assert len(state.active_learning_queue) == 0
