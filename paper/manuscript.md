...
The hybrid oracle was validated against published experimental data for common electrolyte solvents. To maintain a single source of truth and prevent documentation drift, Spearman rank correlation coefficients are generated dynamically.

Run the following command to view the current validation metrics:
```bash
python -m benchmarks.benchmark_external_validation
```

*(For a formatted summary, see `docs/benchmarks.md` in the repository, auto-generated by `scripts/update_benchmark_docs.py`.)*
...
| Metric | Value | Target |
|---|---|---|---|---|
| Mean score gap (discoveries - known) | $+27.98$ | $> 0$ |
