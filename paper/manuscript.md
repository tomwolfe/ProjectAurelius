...
The base energies are calibrated against a reference set of 115 electrolyte molecules:
...
**Quantum DoA:** Penalizes molecules with long conjugation ($L > 12$) and insufficient sp³ carbon support ($f_{\text{sp}^3} < 0.15$), or excessive $\pi$-electron count ($n_\pi > 24$). The conjugation axis uses a continuous sigmoid: $P_{\text{conj}} = 0.70 + 0.30/(1+\exp(2(L-12)))$, which interpolates smoothly between fully in-domain ($\to1.0$ for $L \ll 12$) and the floor of 0.70 for $L \gg 12$. The pi-electron axis uses $P_{\pi} = 0.80 + 0.20/(1+\exp(0.5(n_\pi-24)))$, staying $\approx1.0$ within the calibration domain. The sp³ structural-support condition remains a binary hard gate because it is physically discrete.

**GC DoA:** Penalizes molecules with extreme fluorination without polar solvation sites ($n_{\text{F}} \geq 6 \land n_{\text{polar}} < 2$), high molecular weight ($M_w > 500$), or excessive flexibility (rotatable bonds $> 20$). Each axis uses a continuous sigmoid: $P_{\text{GC}} = 0.75 + 0.25/(1+\exp(0.5(n_F-6)))$ for fluorination, with analogous sigmoids for MW and flexibility. The composite penalty is the product of all active sigmoid penalties, bounded to $[0.75, 1.0]$.
...
- $e_{\text{consist}}$ = external consistency: fraction of top-10 discoveries whose predicted properties fall within the min–max range of known-good electrolytes
...
|---|---|---|---|---|
| Dielectric $\varepsilon$ | 23 | $+0.8493$ | 0.0000 |
...
To ensure the "physics-grounded" claim is falsifiable, we compare the Aurelius oracle against an ECFP4+RandomForest baseline on HOMO and LUMO energies using 5-fold cross-validation. The oracle's value is not raw accuracy but interpretability and extrapolation beyond the training distribution. If the oracle loses to the RF baseline by >0.05 Spearman ρ on any property, we report it honestly rather than gating on it. Results are written to `benchmarks/results/ml_baseline_comparison.json`. The physics-grounded oracle provides chemically meaningful rankings where TOM captures conjugation-driven orbital trends that fragment-based ML models miss, but on HOMO/LUMO specifically, the oracle's ρ may fall below the RF baseline because TOM is a coarse 1-D particle-in-a-box approximation. This is expected and does not undermine the pipeline's core value: the oracle's predictions are physically interpretable and degrade gracefully out-of-domain, whereas black-box ML models produce confident but uninterpretable predictions for novel scaffolds.
